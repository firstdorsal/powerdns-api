(async () => {
    ...
    test("check if non string input results in error", () => {
        expect(() => pdns.deleteRecords(1)).toThrow("records must be of type array");
    });

    test("check if records get deleted", async () => {
        ...
        const d = await pdns.deleteRecords([
            {
                name: zone,
                type: "A"
            }
        ...
        expect(d).toBe(true);
        const e = await pdns.getZone(zone);
